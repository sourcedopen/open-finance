...
<template>
  <div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
        ...
        <div class="card-body">
          ...
          <div class="form-control mt-4">
            <label class="label" for="loanAmount">
              <span class="label-text font-medium">Outstanding Loan Amount (₹)</span>
            </label>
            <input
              id="loanAmount"
      ...
      <div class="card bg-base-200 shadow-xl border border-base-300">
        <div class="card-body">
          ...
          <div class="form-control mt-2">
            <label class="label" for="currentTenure">
              <span class="label-text font-medium">Remaining Tenure (months)</span>
            </label>
            <input
              id="currentTenure"
              v-model.number="currentTenure"
              type="number"
              min="1"
              max="360"
              step="1"
          ...
          <div class="form-control mt-2">
            <label class="label" for="exitCharge">
              <span class="label-text font-medium">Exit / Foreclosure Charge (₹)</span>
            </label>
            <input
              id="exitCharge"
              v-model.number="exitCharge"
              type="number"
              min="0"
              step="500"
      ...
      <div class="card bg-base-200 shadow-xl border border-base-300">
        <div class="card-body">
          ...
          <div class="form-control mt-4">
            <label class="label" for="newRoi">
              <span class="label-text font-medium">Rate of Interest (% p.a.)</span>
            </label>
            <input
              id="newRoi"
              v-model.number="newRoi"
              type="number"
              min="0"
              max="50"
              step="0.1"
          ...
          <div class="form-control mt-2">
            <label class="label" for="newTenure">
              <span class="label-text font-medium">Tenure (months)</span>
            </label>
            <input
              id="newTenure"
              v-model.number="newTenure"
              type="number"
              min="1"
              max="360"
              step="1"
          ...
          <div class="form-control mt-2">
            <label class="label" for="processingCharge">
              <span class="label-text font-medium">Processing Charge (₹)</span>
            </label>
            <input
              id="processingCharge"
              v-model.number="processingCharge"
              type="number"
              min="0"
              step="500"
    ...
    <div
      ...
      class="mb-10 transition-all duration-500"
    >
      <div
        :class="[
          'alert shadow-lg text-lg font-semibold',
          result.isTransferBeneficial ? 'alert-success' : 'alert-error',
        ]"
      >
        <svg
        ...
        >
          <path
            v-if="result.isTransferBeneficial"
        ...
        <div>
          <span v-if="result.isTransferBeneficial">
            ✅ Balance transfer is beneficial! You save
            <span class="font-extrabold">{{ formatCurrency(result.savings) }}</span>
          </span>
          <span v-else>
            ❌ Balance transfer is NOT beneficial. You pay extra
    ...
    >
      ...
      <div class="card bg-base-200 shadow-xl border-t-4 border-error">
        <div class="card-body">
          <h3 class="card-title text-error text-xl">Current Bank Summary</h3>
          <div class="overflow-x-auto mt-4">
            <table class="table table-zebra">
              <tbody>
                <tr>
                  <td class="font-medium">Monthly EMI</td>
                  <td class="text-right text-lg font-bold">{{ formatCurrency(result.current.emi) }}</td>
                </tr>
                <tr>
                  <td class="font-medium">Principal</td>
                  <td class="text-right">{{ formatCurrency(loanAmount) }}</td>
                </tr>
                <tr>
                ...
                <tr>
                  <td class="font-medium">Total Payment</td>
                  <td class="text-right text-lg font-bold text-error">{{ formatCurrency(result.currentTotalCost) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div class="card bg-base-200 shadow-xl border-t-4 border-success">
        <div class="card-body">
          <h3 class="card-title text-success text-xl">New Bank Summary</h3>
          <div class="overflow-x-auto mt-4">
            <table class="table table-zebra">
              <tbody>
                <tr>
                  <td class="font-medium">Monthly EMI</td>
                  <td class="text-right text-lg font-bold">{{ formatCurrency(result.newLoan.emi) }}</td>
                </tr>
                <tr>
                  <td class="font-medium">Principal</td>
                  <td class="text-right">{{ formatCurrency(loanAmount) }}</td>
                ...
                <tr>
                  <td class="font-medium">Exit Charge</td>
                  <td class="text-right text-warning">{{ formatCurrency(exitCharge) }}</td>
                </tr>
                <tr>
                  <td class="font-medium">Processing Charge</td>
                  <td class="text-right text-warning">{{ formatCurrency(processingCharge) }}</td>
                </tr>
                <tr>
                  <td class="font-medium">Total Cost</td>
                  <td class="text-right text-lg font-bold text-success">{{ formatCurrency(result.newTotalCost) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
    ...
    <div
      v-if="result.current.emi > 0 && result.newLoan.emi > 0"
      class="card bg-base-200 shadow-xl mb-10"
    >
      <div class="card-body">
        <h3 class="card-title text-primary text-xl mb-4">Cost Breakdown Comparison</h3>
        <div class="overflow-x-auto">
          <table class="table table-zebra">
            <thead>
              <tr>
                <th>Particulars</th>
                <th class="text-right text-error">Current Bank</th>
                <th class="text-right text-success">New Bank</th>
                <th class="text-right text-primary">Difference</th>
              </tr>
            </thead>
            <tbody>
              <tr>
              ...
              <tr class="font-bold text-lg">
                ...
                <td
                  class="text-right"
                  :class="result.isTransferBeneficial ? 'text-success' : 'text-error'"
                >
